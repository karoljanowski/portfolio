...
const AboutComponent = () => {
    return (
        <div id="about" className="min-h-[100svh] grid grid-cols-2 lg:grid-cols-4 grid-rows-auto grid-flow-dense h-full container mx-auto py-20 px-4 gap-2 lg:gap-3">
            <AboutBox className="col-span-2 bg-gray-900 bg-opacity-30 border-gray-800">
                <HelloBox />
            </AboutBox>
            ...
            <Button href="#projects" className="w-full flex justify-center text-3xl bg-cyan-950 border-cyan-950 text-cyan-500 bg-opacity-30">Projects</Button>

            <AboutBox className="col-span-2 bg-gray-900 bg-opacity-30 border-gray-800">
                <Experience />
            </AboutBox>

            <AboutBox className="bg-blue-950 border-blue-950 bg-opacity-30">
                <Aboutme />
            </AboutBox>
        </div>
    )
...
const AboutBox = ({children, className}: {children?: React.ReactNode, className?: string}) => {
    return (
        <div className={`backdrop-blur rounded-lg border  p-4 lg:p-8 ${className}`}>
            {children}
        </div>
    )
}
